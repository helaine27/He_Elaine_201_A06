...
#iterate through points per game list
for (i in points_per_game){
  #round each element and concatenate to empty vector
  rpoints <- c(rpoints, round(i, 2))
}
...
```{r}
#print(names(max_temps))
for (val in 3:ncol(max_temps)) {
  print(max_temps[val)
}
```
